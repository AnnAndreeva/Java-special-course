...
import com.LR_6.Factories.GamesCollectionFactory;
...
import static com.LR_6.InputOutputGames.createInstance;
import static com.LR_6.InputOutputGames.setGamesIFactory;
import static com.LR_6.InputOutputGamesDB.*;
import static javax.accessibility.AccessibleExtendedText.LINE;
...
public class Main {
    public static void main(String[] args) {
        ...
        do {
            System.out.print("ЛР №4 Андреева 6413\n Меню\n\n" +
                    ...
                    "-------------------------------------------------------------------------------------------\n"+
                    "13) заполнить нитью, считать нитью\n" +
                    "14) write-read-write-read...\n" +
                    "-------------------------------------------------------------------------------------------\n"+
                    "15) Создать объект GamesSeries\n" +
                    "16) Итератор\n" +
                    "17) Создать фабрику\n" +
                    "18) Создать заполненный автоматически неизменяемый объект\n"+
                    "19) Попробовать изменить заголовок неизменяемого объекта\n"+
                    "-------------------------------------------------------------------------------------------\n"+
                    "0) выйти\n\n" +
                    "Выберите пункт меню: ");
            menu = scan.nextLine();
            switch (menu) {
                    ...
                    if (db == null) {
                        ...
                        do {
                            ...
                            if (str.equals("1")) { //создание серии книг
                                ...
                                db[index] = createInstance(title, amountGamesInSeries, amountOfMissionsInPrologue);
                ...
                case "13": {
                    System.out.println("13) заполнить нитью, считать нитью");
                    Random rand = new Random();
                    int amountOfMissionsInPrologue = 3 + rand.nextInt(30 - 3 + 1);


                    // GamesI gamesSeries = new GamesSeries("Новая игровая серия", 4, amountOfMissionsInPrologue);
                    GamesI gamesSeries = new SynchronizedGames( new GamesSeries("Новая игровая серия", 4, amountOfMissionsInPrologue));
                    WritingThread wt = new WritingThread(gamesSeries);
                    ReadingThread rt = new ReadingThread(gamesSeries);
                ...
                case "14": {
                    System.out.println("14) write-read-write-read...");
                    Random rand = new Random();
                    int amountOfMissionsInPrologue = 3 + rand.nextInt(30 - 3 + 1);

                    GamesI gamesCollection = new GamesCollection("Новая коллекция игр", 3, amountOfMissionsInPrologue);

                    GamesSynchronizer gamesSynchronizer = new GamesSynchronizer(gamesCollection);
                    WritingRunnableThread wrt = new WritingRunnableThread(gamesSynchronizer);
                    ReadingRunnableThread rrt = new ReadingRunnableThread(gamesSynchronizer);
                ...
                case "15": {
                    System.out.println("15) Создать объект GamesSeries");
                    gamesI = createInstance("Gothic Series", 4, 2);
                    gamesI.setGames(0, "Gothic");
                    gamesI.setAmountOfMissionsInGame(0, 11);
                    gamesI.setGames(1, "Gothic 2");
                    gamesI.setAmountOfMissionsInGame(1, 14);
                    gamesI.setGames(2, "Gothic 3");
                    gamesI.setAmountOfMissionsInGame(2, 15);
                    gamesI.setGames(3, "Arcania");
                    gamesI.setAmountOfMissionsInGame(3, 17);
                    break;
                }
                case "16": {
                    System.out.println("16) Итератор");
                    if (gamesI == null) {
                        System.out.println("Не существует.\n");
                    } else {
                        for (Integer amountOfMissionInGame : gamesI) {
                            System.out.print(amountOfMissionInGame);
                            System.out.print(' ');
                        }
                        System.out.print('\n');
                    }
                ...
                case "17": {
                    System.out.println("17) Создать фабрику");
                    String menuItem1;
                    Scanner scan1 = new Scanner(System.in);
                    do {
                        System.out.print("Создать объекты следующего типа:\n" +
                                " 1)GamesSeries\n" +
                                " 2)GamesCollection\n" +
                                "Ваш выбор: ");
                        menuItem1 = scan.nextLine();

                        GamesIFactory gamesIFactory;
                        switch (menuItem1) {
                            case "1":
                                gamesIFactory = new GamesSeriesFactory();
                                break;
                            case "2":
                                gamesIFactory = new GamesCollectionFactory();
                                break;
                            default:
                                System.out.println("Выбран неверный пункт меню");
                                continue;
                        ...
                        setGamesIFactory(gamesIFactory);
                        break;
                    } while (true);
                }
                case "18": {
                    System.out.println("18) Создать заполненный автоматически неизменяемый объект");
                    gamesIunmod = InputOutputGames.getUnmodifiableGamesI(gamesI);
                    break;
                }
                case "19": {
                    System.out.println("19) Попробовать изменить заголовок неизменяемого объекта");
                    try {
                        gamesIunmod.setTitle("Divinity");
                    } catch (Exception exc) {
                        System.out.println(exc.getMessage());
                        exc.printStackTrace();
                    }
    ...
    public static GamesI[] createAndFillDbAutomatically() {
        ...
        autoDb[0].setGames(2, "Assassin's Creed 2");
        autoDb[0].setAmountOfMissionsInGame(2, 20);

        autoDb[1] = createInstance("Gothic Series", 4, 2);
        ...
        autoDb[2] = createInstance("The Witcher Series", 3, 5);
        ...
        autoDb[4] = createInstance("The Elder Scrolls Series", 3, 3);
